    ...
    const std::vector<bboxes_ex_msgs::msg::BoundingBox> &bboxes)
{
	...
	const auto it = std::find(ODESSA_CLASSES, ODESSA_CLASSES + 41, bbox.class_id);
        obj.label = std::distance(ODESSA_CLASSES, it);
    ...
    const std::vector<STrack> &trackers)
{
    ...
    for (const auto &tracker : trackers)
    {
        ...
        bbox.class_id = ODESSA_CLASSES[tracker.label];
...
{
    this->video_fps_ = this->declare_parameter<int>("video_fps", 20);
    this->track_buffer_ = this->declare_parameter<int>("track_buffer", 30);
    this->track_thresh_ = this->declare_parameter<double>("track_thresh", 0.25);
    this->high_thresh_ = this->declare_parameter<double>("high_thresh", 0.8);
